...
template <typename T>
inline const T& min(const T& a, const util::optional<T>& b) { return b ? min(a, *b) : a; }

template <typename T>
inline const T& min(const util::optional<T>& a, const T& b) { return a ? min(*a, b) : b; }

template <typename T>
inline const T& min(const util::optional<T>& a, const util::optional<T>& b)
{
	if (!a && !b) {
...
template <typename T>
inline const T& max(const T& a, const util::optional<T>& b) { return b ? max(a, *b) : a; }

template <typename T>
inline const T& max(const util::optional<T>& a, const T& b) { return a ? max(*a, b) : b; }

template <typename T>
inline const T& max(const util::optional<T>& a, const util::optional<T>& b)
{
	if (!a && !b) {
		throw logic_error("Attempt to compare two optionals with no values");
	}
...
constexpr T& update_min(T& minimum_to_update, const util::optional<U>& maybe_new_value)
{
...
template <typename T, typename U = T>
constexpr T& update_max(T& maximum_to_update, const util::optional<U>& maybe_new_value)
{
...
inline string to_string(const util::nullopt_t& n)
{
...
template <typename T, const char* MissingValueIndicator>
inline string to_string(const util::optional<T>& x)
{
...
template <typename T>
inline std::ostream& operator<<(std::ostream& os, const util::optional<T>& x)
{
...
template <typename T>
inline std::ostream& operator<<(std::ostream& os, const util::nullopt_t& n)
{
	return os << std::string("(unset)");
}

template<class T, class ...Args>
T& emplace_or(util::optional<T>& opt, Args&&...args) {
