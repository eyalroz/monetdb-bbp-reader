...
getBBPsize(BBP* bbp)
{
	/* Note: In the original code, this would be an atomic get (of a global) */
	return (bat) bbp->size;
}
...
static int
heapinit(BBP* bbp, BAT *b, const char *buf, int *hashash, unsigned bbpversion, bat bid, const char *filename)
{
	int t;
	char type[33];
	...
	*hashash = var & 2;
	...
	if (bbpversion <= GDKLIBRARY_BLOB_SORT && strcmp(type, "blob") == 0) {
		...
		/* OIDXdestroy(b); */
			/* Not destroying the order index since we don't construct them in this library. */
	}
#endif
...
/* TODO: What about ifdef GDKLIBRARY_HEADED here?*/
		int Thashash;
		...
		nread += heapinit(bbp, bn, buf + nread, &Thashash, bbpversion, bid, filename);
...
/*
	fprintf(stdout, "At end of BBPreadEntries: (oid_nil is " OIDFMT ")\n", oid_nil);
	for(bat bat_index = 292; bat_index < 294; bat_index++) {
		if (BBPvalid(bbp, bat_index)) {
			const BAT *b = BBPdescriptor(bbp, bat_index);
			fprintf(stdout, "BAT record with id %4d, address: %p", (int) bat_index, (void*) b);
			fprintf(stdout, ", cacheid = %d , count = %4d, seq = OIDFMT, COLrec = %p \n",
				(int)  b->batCacheid, (int)  b->batCount, (long long int) b->tseqbase, (void*) &(b->T));
//			VALGRIND_MAKE_MEM_UNDEFINED(b, sizeof(BAT));
			fflush(stdout);
		}
	}
*/
 ...
 * enough */
static gdk_return
BBPcheckbats(BBP* bbp)
{
	for (bat bid = 1; bid < (bat) bbp->size; bid++) {
		struct stat statb;
		BAT *b;
		char *path;

		if ((b = BBP_desc(bbp, bid)) == NULL) {
			/* not a valid BAT */
			continue;
		}
		...
		path = GDKfilepath(bbp->directory, BATDIR, BBP_physical(bbp, b->batCacheid), "tail");
		if (path == NULL)
			return GDK_FAIL;
		...
		GDKfree(path);
		if (b->tvheap != NULL) {
			path = GDKfilepath(bbp->directory, BATDIR, BBP_physical(bbp, b->batCacheid), "theap");
			if (path == NULL)
				return GDK_FAIL;
			if (stat(path, &statb) < 0) {
				GDKsyserror("BBPcheckbats: cannot stat file %s\n",
...
/*	if (rolemask == 0 || (rolemask & 1 && BBPfarms[0].dirname != NULL)) {*/
		/* when only reading from a single DB, farm roles are irrelevant */

	/*
	 * MonetDB tries to create a new farm here if the specified path doesn't exist;
	 * a reader library obviously won't
	 */
...
BBPinit(const char* farm_dir)
{
	...
	/* Not checking for "leftovers" in farms */

	if (BBPcheckbats(bbp) != GDK_SUCCEED)
	...
	if (bbpversion <= GDKLIBRARY_NIL_NAN)
		GDKfatal("Will not change floating-point BATs to use NaN for the null value - run MonetDB on this database first.");
#endif

/*
	fprintf(stdout, "After NIL_NAN, we have:\n");
	for(bat bat_index = 292; bat_index < 294; bat_index++) {
		if (BBPvalid(bbp, bat_index)) {
			const BAT *b = BBPdescriptor(bbp, bat_index);
			fprintf(stdout, "BAT record with id %4d, address: %p", (int) bat_index, (void*) b);
			fprintf(stdout, ", cacheid = %d , count = %4d, seq = %8lld COLrec = %p \n",
				(int)  b->batCacheid, (int)  b->batCount, (long long int) b->tseqbase, (void*) &(b->T));
//			VALGRIND_MAKE_MEM_UNDEFINED(b, sizeof(BAT));
			fflush(stdout);
		}
...
*/
	...
	if (needcommit) {
		GDKfatal(
			"Some fix/change to the persisted data is required before it can be loaded; "
...
BAT *
BBPquickdesc(BBP* bbp, bat bid, bool delaccess)
{
	BAT *b;
