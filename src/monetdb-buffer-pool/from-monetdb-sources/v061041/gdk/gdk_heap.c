 ...
 * In MonetDB code proper, a rmheap parameter allows for removing the heap
 * on freeing it; we don't supprt that. */
void
HEAPfree(Heap *h)
{
	if (h->base) {
	...
	if (h->storage == STORE_MMAPABS)  {
		GDKfatal(
			"We are only reading data from a BBP, a STORE_MMAPABS column should"
			"not be in existence.");
	};
#endif
}
