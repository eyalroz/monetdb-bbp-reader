...
HEAPfree(Heap *h, int remove)
{
	...
	if (h->storage == STORE_MMAPABS)  {
		GDKfatal("We are only reading data from a BBP, a STORE_MMAPABS column should"
			"not be in existence.");
	}
#endif
	h->base = NULL;
	if (h->filename) {
		if (remove) {
			GDKfatal("We are only reading data from a BBP; we should be in a state "
				"in which file removal is required.");
		}
		GDKfree(h->filename);
		h->filename = NULL;
	}
